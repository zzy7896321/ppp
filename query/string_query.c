...
const char* pp_string_query_compare_string[] = {
	"==",
	"!=",
	"<",
	">",
...
int pp_string_query_dump(pp_string_query_t* query, char* buffer, int buf_size) {
	...
	while (query) {
		...
		DUMP(buffer, buf_size, " %s", pp_string_query_compare_string[query->compare]);
...
void pp_string_query_destroy(pp_string_query_t* query) {
	while (query) {
		pp_string_query_t* to_free = query;
		query = query->next;
...
//		fprintf(stderr, "token = %s, value = \"%s\"\n", token_string[token], buffer);
		...
		case INITIAL:
			...
			query = new_pp_string_query(buffer, 0, PP_QUERY_EQ, 0, query);
			state = ID;
			break;
		case ID:
			switch (token) {
	...
	query_lex_destroy(lex);
	return (pp_query_t*) query;
}

static int pp_string_query_acceptor(pp_query_t* _query, pp_trace_t* trace) {
	pp_string_query_t* query = (pp_string_query_t*) _query;
	if (!query) return 1;
